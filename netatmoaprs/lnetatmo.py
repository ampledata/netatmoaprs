...
class ClientAuth:
    """
    Request authentication and keep access token available through token
    method.
...
class DeviceList:
    ...
    def lastData(self, station=None, exclude=0):
        ...
        for mId in s['modules']:
            ds = self.modules[mId].get('dashboard_data')

            if ds and ds['time_utc'] > limit:
...
if __name__ == '__main__':
    ...
    if not _CLIENT_ID or not _CLIENT_SECRET or not _USERNAME or not _PASSWORD :
           stderr.write(
            "Library source missing identification arguments to " +
            "check lnetatmo.py (user/password/etc...)"
            )
           exit(1)
