# Makefile for NetatmoAPRS Python Module.
#
...
pep8: install_requirements
	flake8 --max-complexity 12 --exit-zero netatmoaprs/*.py tests/*.py

lint: install_requirements
	pylint --msg-template="{path}:{line}: [{msg_id}({symbol}), {obj}] {msg}" \
		-r n netatmoaprs/*.py tests/*.py || exit 0
